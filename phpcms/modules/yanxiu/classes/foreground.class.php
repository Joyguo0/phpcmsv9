	...
	final public function check_member() {
		...
		if(ROUTE_M =='member' && ROUTE_C =='index' && in_array(ROUTE_A, array('login', 'register', 'mini','send_newmail'))) {
			if ($phpcms_auth && ROUTE_A != 'mini') {
				showmessage(L('login_success', '', 'member'), 'index.php?m=yanxiu&c=index');
			} else {
				return true;
			}
		} else {
			...
			if ($phpcms_auth) {
				...
				if($this->memberinfo && $this->memberinfo['password'] === $password) {
					...
					if($this->memberinfo['groupid'] == 1) {
						...
						showmessage(L('userid_banned_by_administrator', '', 'member'), 'index.php?m=yanxiu&c=index&a=login');
						...
						showmessage(L('need_emial_authentication', '', 'member'), 'index.php?m=yanxiu&c=index&a=register&t=2');
					}
				} else {
					param::set_cookie('auth', '');
					param::set_cookie('_userid', '');
				...
				showmessage(L('please_login', '', 'member'), 'index.php?m=yanxiu&c=index&a=login&forward='.$forward);
			}
		}
	}
	/**
