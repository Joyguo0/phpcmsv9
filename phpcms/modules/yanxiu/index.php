...
class index extends foreground {
	...
	public function register() {
		...
		if(!$member_setting['allowregister']) {
			showmessage(L('deny_register'), 'index.php?m=yanxiu&c=index&a=login');
		}
		//加载短信模块配置
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($member_setting['enablemailcheck']) {	//是否需要邮件验证
				...
				showmessage(L('operation_success'), 'index.php?m=yanxiu&c=index&a=register&t=3');
			} else {
				//查看当前模型是否开启了短信验证功能
			...
			if(pc_base::load_config('system', 'phpsso')) {
				...
				if($status > 0) {
					...
					if($member_setting['enablemailcheck']) {
						...
						$url = APP_PATH."index.php?m=yanxiu&c=index&a=register&code=$code&verify=1";
						...
						showmessage(L('operation_success'), 'index.php?m=yanxiu&c=index&a=register&t=2');
					} else {
						//如果不需要邮箱认证、直接登录其他应用
						$synloginstr = $this->client->ps_member_synlogin($userinfo['phpssouid']);
						showmessage(L('operation_success').$synloginstr, 'index.php?m=yanxiu&c=index&a=init');
					}
					
				}
			} else {
				showmessage(L('enable_register').L('enable_phpsso'), 'index.php?m=yanxiu&c=index&a=login');
			}
			...
			if(!empty($_GET['verify'])) {
				$code = isset($_GET['code']) ? trim($_GET['code']) : showmessage(L('operation_failure'), 'index.php?m=yanxiu&c=index');
				$phpcms_auth_key = md5(pc_base::load_config('system', 'auth_key'));
				$code_res = sys_auth($code, 'DECODE', $phpcms_auth_key);
				$code_arr = explode('|', $code_res);
				$userid = isset($code_arr[0]) ? $code_arr[0] : '';
				$userid = is_numeric($userid) ? $userid : showmessage(L('operation_failure'), 'index.php?m=yanxiu&c=index');

				$this->db->update(array('groupid'=>$this->_get_usergroup_bypoint()), array('userid'=>$userid));
				showmessage(L('operation_success'), 'index.php?m=yanxiu&c=index');
	...
	public function account_manage_upgrade() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($memberinfo['amount'] >= $cost) {
				...
				showmessage(L('operation_success'), 'index.php?m=yanxiu&c=index&a=init');
			} else {
				showmessage(L('operation_failure'), HTTP_REFERER);
			}
	...
	public function login() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			$tst=time();
			$secret_key='58d@%sdf$547DSKf14(sd2Ls';
			$data=array('tp'=>'61001','pp'=>$username,'pwd'=>md5($password),'tst'=>$tst,'key'=>md5(($username+$password+'61001'+$tst+$secret_key)));			
			//print_r($data);
			$yanxiu_return_msg= $this->postdatacurl($data, 'http://pp.yanxiu.com/reg/loginValidate.tc');
			//print_r($yanxiu_return_msg);die;
			if($yanxiu_return_msg->code!=0){
				//showmessage($yanxiu_return_msg->desc, HTTP_REFERER);
			}
			if(!isset($cookietime)) {
				$get_cookietime = param::get_cookie('cookietime');
			}
			...
			param::set_cookie('_nickname', $username, $cookietime);
			//param::set_cookie('cookietime', $_cookietime, $cookietime);
			$forward = isset($_POST['forward']) && !empty($_POST['forward']) ? urldecode($_POST['forward']) : 'index.php?m=yanxiu&c=index';
			showmessage(L('login_success').$username, $forward);
	...
	public function logout() {
		...
		if($setting['snda_enable'] && param::get_cookie('_from')=='snda') {
			...
			$forward = isset($_GET['forward']) && trim($_GET['forward']) ? urlencode($_GET['forward']) : '';
			$logouturl = 'https://cas.sdo.com/cas/logout?url='.urlencode(APP_PATH.'index.php?m=yanxiu&c=index&a=logout&forward='.$forward);
			...
			$forward = isset($_GET['forward']) && trim($_GET['forward']) ? $_GET['forward'] : 'index.php?m=yanxiu&c=index&a=login';
		...
		if(isset($_GET['callback']) && trim($_GET['callback'])) {
			...
			if(!empty($me['id'])) {
				...
				if(!empty($r)) {
					...
					param::set_cookie('cookietime', $_cookietime, $cookietime);
					param::set_cookie('_nickname', $nickname, $cookietime);
					$forward = isset($_GET['forward']) && !empty($_GET['forward']) ? $_GET['forward'] : 'index.php?m=yanxiu&c=index';
				...
				showmessage(L('login_failure'), 'index.php?m=yanxiu&c=index&a=login');
			}
		} else {
			$o = new WeiboOAuth(WB_AKEY, WB_SKEY);
			$keys = $o->getRequestToken();
			$aurl = $o->getAuthorizeURL($keys['oauth_token'] ,false , APP_PATH.'index.php?m=yanxiu&c=index&a=public_sina_login&callback=1');
			$_SESSION['keys'] = $keys;
	...
	public function public_snda_login() {
		...
		define('SNDA_CALLBACK', urlencode(APP_PATH.'index.php?m=yanxiu&c=index&a=public_snda_login&callback=1'));
	...
	public function public_qq_login() {
		...
		if(isset($_GET['callback']) && trim($_GET['callback'])) {
			...
			$oauth_callback = APP_PATH.'index.php?m=yanxiu&c=index&a=public_qq_login&callback=1';
	...
	public function public_forget_password () {
		...
		if(isset($_POST['dosubmit'])) {
			...
			$url = APP_PATH."index.php?m=yanxiu&c=index&a=public_forget_password&code=$code";
			...
			showmessage(L('operation_success'), 'index.php?m=yanxiu&c=index&a=login');
		} elseif($_GET['code']) {
			$phpcms_auth_key = md5(pc_base::load_config('system', 'auth_key').$this->http_user_agent);
			...
			if(is_array($code) && is_numeric($code[0]) && date('y-m-d h', SYS_TIME) == date('y-m-d h', $code[1])) {
				...
				if(empty($memberinfo['phpssouid'])) {
					showmessage(L('operation_failure'), 'index.php?m=yanxiu&c=index&a=login');
				}
				$updateinfo = array();
				$password = random(8);
